...
#   --color=auto:              Colorea la salida. Puede ser 'always' (por defecto si se omite), 'auto', o 'never'.
#   --context:                 Imprime el contexto de seguridad de cada archivo
#   --format:                  Admite across (-x), commas (-m), horizontal (-x), long (-l), single-column (-1), verbose (-l), vertical (-C)
