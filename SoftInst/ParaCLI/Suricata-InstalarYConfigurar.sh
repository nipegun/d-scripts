              ...
              if [ $(echo ${#aInterfacesActivasConIP[@]}) == "1" ]; then
                ...
                # Crear reglas básica
                  ...
                  # Bloquear todas las conexiones entrantes no establecidas
                    echo 'drop icmp $EXTERNAL_NET any -> $HOME_NET any (msg: "Bloqueada conexión ICMP entrante no establecida"; flow:not_established; sid:5000002; rev:1;)' >> /etc/suricata/rules/debian-cortafuegos.rule
                    echo 'drop tcp $EXTERNAL_NET any -> $HOME_NET any (msg: "Bloqueada conexión TCP entrante no establecida"; flow:not_established; sid:5000002; rev:1;)' >> /etc/suricata/rules/debian-cortafuegos.rule
                    echo 'drop udp $EXTERNAL_NET any -> $HOME_NET any (msg: "Bloqueada conexión UDP entrante no establecida"; flow:not_established; sid:5000003; rev:1;)' >> /etc/suricata/rules/debian-cortafuegos.rule
