...
rman target / << EOF
run {
backup incremental level 1 database format            '$vCarpetaCopSeg/%d-${DATE}-database_%u';
backup incremental level 1 archivelog all format      '$vCarpetaCopSeg/%d-${DATE}-archivelogs_%u' delete input;
backup incremental level 1 current controlfile format '$vCarpetaCopSeg/%d-${DATE}-controlfile_%u';
}
exit;
EOF
