...
# Ejecutar comandos dependiendo de la versión de Debian detectada
  ...
  if [ $cVerSO == "13" ]; then
    ...
    # Credenciales iniciales admin:adin
      ...
      echo ''
      echo '    Para agregar un dispositivo (Por ejemplo un sensor DraginoDHT52):'
      echo '      Primero agregamos el perfil para el dispositivo:'
      echo '        "Device profiles" > "Add device profile"'
      echo '          Pestaña General'
      echo '            Name: eu868-otaa-class-a'
      echo '            Region: EU868'
      echo '            MAC version: LoRaWAN 1.0.3'
      echo '            Regional parameters revision: RP002-1.0.3'
      echo '            ADR algorithm: Default ADR algorithm'
      echo '            Flush queue on activate: activado'
      echo '            Expected Uplink interval: 3600'
      echo '          Pestaña Join (OTAA / ABP)'
      echo '            Supports OTAA: activado'
      echo '          Pestaña Class-B'
      echo '            Supports Class-B: desactivado'
      echo '          Pestaña Class-C'
      ...
      echo '          Pestaña Measurements:'
      echo '            Modificamos:'
      echo '              Hum_SHT    → Gauge    → Humedad'
      echo '              TempC_SHT  → Gauge    → Temperatura'
      echo '            Borramos:'
      echo '              Ext          → Unknown / unset'
      echo '              Node_type    → Unknown / unset'
      echo '              Systimestamp → Unknown / unset'
      echo '              TempC_DS     → Unknown / unset'
      echo '            Sólo activamos:'
      echo '              TempC_DS     → Unknown / unset  (si le conectamos una sonda externa DS18B20)'
      echo '      Luego agregamos la aplicación:'
      echo '        "Applications" > "sensores" > "Devices" > "Add device"'
      echo '          Pestaña "Device"'
      echo '            Name: DraginoLHT52'
      echo '            Device EUI: Dev EUI del sensor, sin dos puntos (ejemplo: 70b3d57ed006abcd)'
      echo '            Join EUI (EUI64): El APP EUI'
      echo '            Device profile: eu868-otaa-class-a'
      echo '          Pestaña "OTAA Keys"'
      echo '            Application key: AppKey del sensor, 32 caracteres HEX (ejemplo: 00112233445566778899aabbccddeeff)'
      echo ''
