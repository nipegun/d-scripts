...
if [ "$cVerSO" == "13" ]; then
  ...
  fRunPreInstallChecks() {
    ...
    if [ -d "$cBookstackDir" ]; then
      if [ "$(find "$cBookstackDir" -mindepth 1 -maxdepth 1 | head -n 1)" ]; then
        fErrorOut "Ya existe ${cBookstackDir} y no está vacío"
      fi
    fi

    if id "$cBookstackUser" >/dev/null 2>&1; then
      fErrorOut "Ya existe el usuario de sistema ${cBookstackUser}"
    fi

    if command -v ss >/dev/null 2>&1; then
      if ss -ltn | sed -n "s/.*:${cHttpPort} .*/1/p" | grep -q "1"; then
        fErrorOut "El puerto HTTP ${cHttpPort} ya está en uso"
      fi

      if ss -ltn | sed -n "s/.*:${cHttpsPort} .*/1/p" | grep -q "1"; then
        fErrorOut "El puerto HTTPS ${cHttpsPort} ya está en uso"
      fi
    fi
  }
  ...
  fRunPromptForDomainIfRequired() {
    ...
    vDominio="$(echo "$vDominio" | sed "s#:${cHttpPort}$##")"
    vDominio="$(echo "$vDominio" | sed "s#:${cHttpsPort}$##")"
  }
  ...
  fRunUpdateBookstackEnv() {
    ...
    fRunSetEnvValue "APP_URL" "https://${vDominio}:${cHttpsPort}"
  ...
  fRunSetApplicationFilePermissions() {
    ...
    sudo git -C "$cBookstackDir" config --global --add safe.directory "$cBookstackDir"
  ...
  fRunConfigureApacheServerName() {
    echo "ServerName ${vDominio}" | sudo tee /etc/apache2/conf-available/bookstack-servername.conf > /dev/null
  ...
  fRunConfigureApache() {
    ...
    sudo systemctl start php${cPhpVersion}-fpm.service
  ...
  fInfoMsg "- Puerto HTTP: ${cHttpPort}"
  fInfoMsg "- Puerto HTTPS: ${cHttpsPort}"
  fInfoMsg ""
  ...
  fInfoMsg "- URL HTTP: http://${vDominio}:${cHttpPort}/"
  fInfoMsg "- URL HTTPS: https://${vDominio}:${cHttpsPort}/"
  fInfoMsg "- APP_URL configurado: https://${vDominio}:${cHttpsPort}"
  fInfoMsg "- Ruta de instalación: ${cBookstackDir}"
  fInfoMsg "- Usuario de sistema: ${cBookstackUser}"
  fInfoMsg "- Home del usuario ${cBookstackUser}: ${cBookstackDir}"
